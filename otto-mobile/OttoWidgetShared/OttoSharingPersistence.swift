...
public enum OttoSharingUserDefaultsKeys {
    ...
    /// Whether sharing sessions save drive path/history (Sharing sheet “Save this Drive”).
    public static let sharingSaveDriveEnabled = "otto.sharingSaveDriveEnabled"
...
/// Home-screen location sharing widget (WidgetKit).
///
/// When `false` (default): the app does not embed `OttoSharingWidgetExtension`, skips widget timeline reloads,
/// and does not geocode for the widget. Set to `true` and re-add the extension to **Embed App Extensions** when ready to ship.
public enum OttoSharingWidgetConfiguration {
...
public enum OttoSharingPersistence {
    ...
    public static func mirrorSharingKeysToStandard(from suite: UserDefaults = OttoAppGroup.suite) {
        ...
        if suite.object(forKey: OttoSharingUserDefaultsKeys.sharingSaveDriveEnabled) != nil {
            standard.set(suite.bool(forKey: OttoSharingUserDefaultsKeys.sharingSaveDriveEnabled), forKey: OttoSharingUserDefaultsKeys.sharingSaveDriveEnabled)
        } else {
            standard.removeObject(forKey: OttoSharingUserDefaultsKeys.sharingSaveDriveEnabled)
        }
